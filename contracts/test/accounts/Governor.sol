...
contract Governor {
    ...
    function distributeToHolders(IMapleTreasury treasury)                              external { treasury.distributeToHolders(); }
    ...
    function try_reclaimERC20_treasury(address treasury, address asset, uint256 amount) external returns (bool ok) {
        string memory sig = "reclaimERC20(address,uint256)";
        (ok,) = address(treasury).call(abi.encodeWithSignature(sig, asset, amount));
    }

    function try_distributeToHolders(address treasury) external returns (bool ok) {
        string memory sig = "distributeToHolders()";
        (ok,) = address(treasury).call(abi.encodeWithSignature(sig));
    }

    function try_convertERC20(address treasury, address asset) external returns (bool ok) {
